const ALLOWED = {
  ...
  "auto-social-post": "generate.yml",
  "insta_analytics": "sync.yml"
};
...
export default {async fetch(request,env){
  ...
  const gh=await fetch(url,{method:"POST",headers:{"Accept":"application/vnd.github+json","Authorization":`Bearer ${env.GH_TOKEN}`,"X-GitHub-Api-Version":"2026-03-10","User-Agent":"ai-command-center"},body:JSON.stringify({ref:"main"})});
  if(!gh.ok){const text=await gh.text();return json({error:`GitHub API ${gh.status}`,details:text.slice(0,500)},gh.status)}
